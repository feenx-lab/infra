...
module "config" {
  ...
  node_names     = [for n in var.cluster_nodes : n.hostname]
  node_ips       = [for n in var.cluster_nodes : n.ip if n.role == "controlplane"]
...
module "nx0" {
  ...
  # Inline Manifests
  inline_manifests = module.cilium.manifests
}

module "nx1" {
  depends_on = [module.nx0]
  source     = "../../modules/node"
